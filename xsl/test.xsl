<?xml version="1.0"?>
<!--xsl:stylesheet定义样式表的根元素-->
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	<!--xsl:output定义输出文档的格式-->
	<xsl:output doctype-public="-//W3C//DTD HTML 4.01 Transitional//EN" encoding="UTF-8" indent="yes" method="html" doctype-system="http://www.w3.org/TR/html4/loose.dtd"/>
	<xsl:strip-space elements="*"/>
	<!--xsl:param声明参数-->
	<xsl:param select="'API Test Result'" name="titleReport"/>

	<xsl:param select="'date not defined'" name="dateReport"/>
	...
	<xsl:template match="testResults">
		<html>
			<head>
				...
				<style type="text/css"><![CDATA[
	                * { margin: 0; padding: 0 }
	                ...
	                #left-panel { float:left; margin-top: 20px; margin-left: 20px; width: 300px; overflow: auto; background: #dee4ea; padding: 20px 20px 0;}
	                ...
	                #right-panel { margin-top: 20px; margin-right: 20px; overflow: auto; background: white }
				...
				<style type="text/css"><![CDATA[
					#head-coontent table tr td, table tr th {font-size: 68%;}table.details tr th{color: #ffffff;font-weight: bold;text-align:center;background:#2674a6;white-space: nowrap;}#head-coontent table.details tr td{background:#dee4ea;white-space: nowrap;}h1 {margin: 0px 0px 5px; font: 165% verdana,arial,helvetica}h2 {margin-top: 1em; margin-bottom: 0.5em; font: bold 125% verdana,arial,helvetica}h3 {margin-bottom: 0.5em; font: bold 115% verdana,arial,helvetica}.Failure {font-weight:bold; color:red;}#head-coontent {width: 100%; padding: 20px 20px 0; box-sizing: border-box;}#head-coontent table.details tr td { font-size: 12px; padding: 8px; text-align: center;}#head-coontent table.details tr th { font-size: 12px; padding: 8px; }
					]]>
				</style>
	            ...
	            <script type="text/javascript"><![CDATA[
	                ...
	                var onclick_li = (function() {
	                    ...
	                    return function(li) {
	                        ...
	                        document.getElementById("right-panel").innerHTML = last_selected.lastElementChild.innerHTML;
	                        return false;
	                    };
	                })();
			...
			<body>
				<div id="head-coontent">
					<!--xsl:call-template调用一个指定的模板pageHeader-->
	                               ...
	                               <div class="zebra">
	                               	<table>
	                               		<tr>
	                                        ...
	                                        <td class="data"><xsl:value-of select="@tn"/></td>
	                                    </tr>
	                                    <tr>
	                                    ...
	                                    <tr>
	                                        ...
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@t"/> ms</td>
	                                    </tr>
	                                    <tr>
	                                        <td class="data key">Latency</td>
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@lt"/> ms</td>
	                                    </tr>
	                                    <tr>
	                                        <td class="data key">Bytes</td>
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@by"/></td>
	                                    </tr>
	                                    <tr>
	                                        <td class="data key">Sample Count</td>
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@sc"/></td>
	                                    </tr>
	                                    <tr>
	                                        <td class="data key">Error Count</td>
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@ec"/></td>
	                                    </tr>
	                                    <tr>
	                                        <td class="data key">Response Code</td>
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@rc"/></td>
	                                    </tr>
	                                    <tr>
	                                        <td class="data key">Response Message</td>
	                                        <td class="data delimiter">:</td>
	                                        <td class="data"><xsl:value-of select="@rm"/></td>
	                                    </tr>
	                                </table>
	                            </div>
	                            <div class="trail"></div>
	                                        ...
	                                        <table>
	                                            <xsl:for-each select="assertionResult">
	                                                <tbody>
	                                                    ...
	                                                    <tr>
	                                                        <td class="data assertion" colspan="3"><xsl:value-of select="name"/></td>
	                                                    </tr>
	                                                    <tr>
	                                                        <td class="data key">Failure</td><td class="data delimiter">:</td>
	                                                        <td class="data"><xsl:value-of select="failure"/></td>
	                                                    </tr>
	                                                    <tr>
	                                                        <td class="data key">Error</td><td class="data delimiter">:</td>
	                                                        <td class="data"><xsl:value-of select="error"/></td>
	                                                    </tr>
	                                                    <tr>
	                                                        <td class="data key">Failure Message</td><td class="data delimiter">:</td><td class="data"><xsl:value-of select="failureMessage"/></td>
	                                                    </tr>
	                                                </tbody>
	                                            </xsl:for-each>
	                                        </table>
...
<xsl:template name="summary">
	<h2>Summary</h2>
	<table align="center" width="100%" cellspacing="2" cellpadding="5" border="0" class="details" border-collapse="separate" border-spacing="2px">
		<tr valign="top">
			<th>All</th>
			<th>Failures</th>
			<th>Success</th>
		...
		<tr valign="top">
			<!--声明变量，并向变量添加值-->
			<xsl:variable select="count(/testResults/*)" name="allCount"/>
			<xsl:variable select="count(/testResults/*[attribute::s='false'])" name="allFailureCount"/>
			<xsl:variable select="count(/testResults/*[attribute::s='true'])" name="allSuccessCount"/>
			<xsl:variable select="$allSuccessCount div $allCount" name="allSuccessPercent"/>
			...
			<td align="center">
				<!--xsl:call-template调用一个指定的模板display-percent-->
				<xsl:call-template name="display-percent">
					<xsl:with-param select="$allSuccessPercent" name="value"/>
				</xsl:call-template>
			</td>
		</tr>
	</table>
...
<xsl:template name="pagelist">
	<h2>Pages</h2>
	<table align="center" width="100%" cellspacing="2" cellpadding="5" border="0" class="details">
		<tr valign="top">
			<th>URL</th>
			<th>All</th>
			<th>Failures</th>
		...
		<xsl:for-each select="/testResults/*[not(@lb = preceding::*/@lb)]">
			<xsl:variable select="@lb" name="label"/>
			<xsl:variable select="count(../*[@lb = current()/@lb])" name="count"/>
			<xsl:variable select="count(../*[@lb = current()/@lb][attribute::s='false'])" name="failureCount"/>
			<xsl:variable select="count(../*[@lb = current()/@lb][attribute::s='true'])" name="successCount"/>
			<xsl:variable select="$successCount div $count" name="successPercent"/>
			<tr valign="top">
				<xsl:attribute name="class">
					<xsl:choose>
				...
				<td align="center">
					<xsl:call-template name="display-percent">
						<xsl:with-param select="$successPercent" name="value"/>
					</xsl:call-template>
				</td>
			</tr>
		</xsl:for-each>
